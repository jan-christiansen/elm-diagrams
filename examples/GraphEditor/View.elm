...
nodeXOutActions nodeId =
    { emptyActionSet | click = Just <| keepBubbling <| always <| [ RemoveNode nodeId ] }


edgeXOutActions edge =
    { emptyActionSet | click = Just <| keepBubbling <| always <| [ RemoveEdge edge ] }
...
canvasActions dragState =
    let
        dragMove =
            { emptyActionSet
                | mouseMove = Just <| stopBubbling <| \(MouseEvent evt) -> [ DragMove evt.offset ]
            ...
            { emptyActionSet
                | mouseUp =
                    Just <|
                        stopBubbling <|
                            always <|
                                [ AddEdge { from = attrs.fromPort, to = portId } ]
            }
...
xGlyph =
    let
        smallLine =
            vline 11 { defLine | color = Color.white, width = 2 }

        rotLeft =
            rotate (-pi / 4) smallLine

        rotRight =
            rotate (pi / 4) smallLine

        bg =
            circle 7 <| justFill <| Solid Color.red
    in
    zcat [ rotLeft, rotRight, bg ]
...
viewPosNode dState pn =
    move pn.pos <| tagWithActions (NodeIdT pn.id) (posNodeActions pn.id dState) <| viewNode pn.node pn.id dState
...
viewNode node nodeId dState =
    let
        -- top row
        title =
            text titleStyle node.title

        xOut =
            tagWithActions XOut (nodeXOutActions nodeId) <| xGlyph

        titleRow =
            flexCenter title xOut

        -- ports
        portCirc =
            circle 7 (justFill <| Solid Color.yellow)

        label lbl =
            text T.defaultStyle lbl

        -- in ports
        inSlot lbl =
            flexRight <| hcat [ tagWithActions (InPortT lbl) (inPortActions ( nodeId, lbl ) dState) <| portCirc, hspace 5, label lbl ]

        inSlots =
            L.map inSlot node.inPorts

        -- out ports
        outSlot lbl =
            flexLeft <| hcat [ label lbl, hspace 5, tagWithActions (OutPortT lbl) (outPortActions ( nodeId, lbl )) portCirc ]

        outSlots =
            L.map outSlot node.outPorts

        -- pad
        padded =
            padSpecific 5 5 7 7 <| layout <| [ titleRow ] ++ inSlots ++ outSlots
    in
    background (fillAndStroke (Solid Color.orange) defaultStroke) padded
...
viewEdgeXOut nodesDia edge =
    let
        edgeCoords =
            getEdgeCoords nodesDia edge
    in
    tagWithActions XOut (edgeXOutActions edge) <| move edgeCoords.to <| xGlyph
...
view state =
    let
        nodes =
            zcat <| L.map (viewPosNode state.dragState) <| D.values state.graph.nodes

        edges =
            zcat <| L.map (viewEdge nodes) state.graph.edges

        edgeXOuts =
            zcat <| L.map (viewEdgeXOut nodes) state.graph.edges
        ...
        draggingEdge =
            case state.dragState of
                ...
                _ ->
                    []
    in
    tagWithActions Canvas (canvasActions state.dragState) <| pad 10000 <| zcat <| draggingEdge ++ [ edgeXOuts, edges, nodes ]
