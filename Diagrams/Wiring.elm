...
--     let
--         collageLocs =
--             S.map clf floatWindowDims
--
--         mouseEvts =
--             mouseEvents collageLocs
--
--         tups =
--             S.map2 (,) collageLocs mouseEvts
--     in
--     S.filter (\( loc, ( evtType, point ) ) -> pointInside point { loc | offset = ( 0, 0 ) })
...
--
--         moves =
--             S.map (always MouseMoveEvt) Mouse.position
--
--         events =
--             S.merge upDown moves
--
--         adjustedMousePos =
--             S.map2 offsetMousePos loc floatMousePos
--     in
--     S.map2 (,) events adjustedMousePos
...
offsetMousePos loc ( x, y ) =
    let
        ...
        { width, height } =
            loc.dims
    in
    ( x - width / 2 - offsetX, (height / 2 + offsetY) - y )
...
-- floatMousePos =
--     S.map toPoint Mouse.position
-- {-| -}
-- floatWindowDims : Signal Dims
-- floatWindowDims =
--     S.map (\( w, h ) -> { width = toFloat w, height = toFloat h })
--         Window.dimensions
