...
{-| -}
type alias RenderFunc m t a =
    m -> Diagram t a


{-| -}
type alias UpdateFunc m a =
    a -> m -> m
...
processMouseEvent diagram mouseState ( evt, mousePos ) =
    let
        overTree =
            pick diagram mousePos

        -- need to pick every time because actions may have changed
        overPickedTags =
            preorderTraverse overTree

        overPaths =
            tagPaths overPickedTags

        oldOverPickedTags =
            mouseState.overPickedTags

        oldOverPaths =
            tagPaths oldOverPickedTags
    in
    case evt of
        MouseMoveEvt ->
            let
                enters =
                    L.filterMap (getHandler .mouseEnter) <|
                        L.filter (\pTag -> not <| L.member (tagPath pTag) oldOverPaths) overPickedTags

                leaves =
                    L.filterMap (getHandler .mouseLeave) <|
                        L.filter (\pTag -> not <| L.member (tagPath pTag) overPaths) oldOverPickedTags

                moves =
                    L.filterMap (getHandler .mouseMove) <|
        ...
        MouseUpEvt ->
            let
                ...
                clicks =
                    if overPaths == M.withDefault [] mouseState.overPathsOnMouseDown then
                        L.filterMap (getHandler .click) overPickedTags
                    else
                        []
            in
            ( { mouseState
    ...
    let
        recurse path tree =
            case tree of
                ...
                PickLayers layers ->
                    L.concatMap (recurse path) layers
    in
    case maybeTree of
        Just tree ->
            recurse [] tree
...
getHandler :
    (ActionSet t a -> Maybe (EventToAction t a))
    -> PickedTag t a
