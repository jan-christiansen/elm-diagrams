...
bezier a cp1 cp2 b ls =
    path (bezierCurve a cp1 cp2 b) ls


bezierCurve : Point -> Point -> Point -> Point -> List Point
bezierCurve a cp1 cp2 b =
    L.map (\x -> bezierPoint x [ a, cp1, cp2, b ]) resolution
...
bezierPoint t points =
    case points of
        ...
        x :: xs ->
            bezierPoint t (L.map2 (interpolatePoint t) points xs)
...
interpolatePoint t ( x0, y0 ) ( x1, y1 ) =
    ( lerp ( x0, x1 ) ( 0, 1 ) t, lerp ( y0, y1 ) ( 0, 1 ) t )
