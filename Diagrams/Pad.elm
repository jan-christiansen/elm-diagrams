...
padSpecific u d l r dia =
    let
        ...
        offsetDims =
            bbox2offsetDims paddedBbox

        padder =
            move offsetDims.offset <| spacer offsetDims.dims.width offsetDims.dims.height
    in
    zcat [ dia, padder ]
...
background fs dia =
    let
        offsetDims =
            bbox2offsetDims <| boundingBox dia

        bg =
            move offsetDims.offset <| rect offsetDims.dims.width offsetDims.dims.height fs
    in
    zcat [ dia, bg ]
...
outlineBox ls dia =
    background (justStroke ls) <| pad (ls.width / 2) dia
