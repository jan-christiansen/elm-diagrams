...
type alias LayoutRow t a =
    List (FlexDiagram t a)
...
layout rows =
    let
        decidedWidth =
            L.map rowMinWidth rows |> L.maximum |> M.withDefault 0
    in
    vcatA LeftA <| L.map (renderRow decidedWidth) rows
...
renderRow decidedWidth row =
    let
        ...
        numExpandos =
            L.sum <| L.map count row

        leftOver =
            decidedWidth - L.sum hardMins

        eachExpando =
            if numExpandos > 0 then
                leftOver / toFloat numExpandos
            else
                0
        ...
        renderFD fd =
            case fd of
                ...
                Expando { minWidth, render } ->
                    render (minWidth + eachExpando)
    in
    hcat <| L.map renderFD row
...
spring =
    Expando { minWidth = 0, render = hspace }
...
flexCenter ldia rdia =
    [ block ldia, spring, block rdia ]
...
hrule ls vPadding =
    flexAll (\w -> padSpecific vPadding vPadding 0 0 <| hline w ls)
