...
eqTriangle sideLength fs =
    ngon 3 sideLength fs
...
ngon n r fs =
    ...
    in
    polygon (L.map f (List.range 0 (floor (m - 1)))) fs
                ...
                filled =
                    case fs.fill of
                        Just fillStyle ->
                            case fillStyle of
                                Solid color ->
                                    [ C.filled color shape ]

                                Texture src ->
                                    [ C.textured src shape ]

                                Grad grad ->
                                    [ C.gradient grad shape ]
